...
def eval_det_cls(pred, gt, ovthresh=0.25, use_07_metric=False, get_iou_func=get_iou):
    ...
    fp = np.cumsum(fp)
    tp = np.cumsum(tp)
    print('False positives:', fp, 'True positives: ', tp)
    rec = tp / float(npos)
    # print('NPOS: ', npos)
...
def eval_det(pred_all, gt_all, ovthresh=0.5, use_07_metric=False, get_iou_func=get_iou):
...
if __name__ == '__main__':
    ...
    VALID_CLASS_IDS = [1, 2]
    data_path = './dataset/applesTFM/visu/'
    prediction_path = './prediction/baseTest12Eps/pred_instance'
    ...
    def single_process(instance_path):
        ...
        gt = osp.join(data_path, img_id + '.las')  # 0-based index
        assert osp.isfile(gt)
        coords, rgb, semantic_label, instance_label = loadAppleDataSet(gt)
    ...
    rec, prec, ap = eval_det(pred_all,gt_all)
    print('Recall',rec)
    print('Precision',rec)
